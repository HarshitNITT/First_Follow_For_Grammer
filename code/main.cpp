    ...
    if(isupper(str[index]))
    {
        ...
        loop(0,y,cnt[str[index]])
        {
            if(!visited[exp[str[index]][y]])
            {
                ...
                calc_first(str[index],exp[str[index]][y],2,visited,exp,cnt,first);
...
void print(vc<char>nt_unique,u_map<char,vc<char>>&follow_final,int cnt)
{
    loop(0,i,nt_unique.size())
    {
        cout<<nt_unique[i]<<" -----------> ";
...
int main()
{
    ...
    loop(0,i,num_productions)
    {
        ...
        {
            visited[prod[i]]=true;
            calc_first(prod[i][0],prod[i],2,visited,exp,cnt,first);
            // cout<<i<<endl;
        }
    }
    ...
    loop(0,i,nt_unique.size())
    sort(follow[nt_unique[i]].begin(),follow[nt_unique[i]].end());
    removeDuplicate(follow,follow_final,nt_unique);
    removeDuplicate(first,first_final,nt_unique);
    cout<<"Follow:"<<endl;
    print(nt_unique,follow_final,1);
    cout<<"First"<<endl;
    print(nt_unique,first_final,0);
}
// 
